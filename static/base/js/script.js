		...
		if(!that.hasClass('optgroup')) {
            ...
            $.ajax({
                ...
                success: function(json){
                    if (data.language == 'vina') {
                        location.href= location.pathname.replace('/en/', '/vi/')
                    } else {
                        location.href= location.pathname.replace('/vi/', '/en/')
                    }
                },
                error: function(){
                    console.log('error')
...
function sign_in_form_submit(){
    console.log('sing-in-form submitted!')
    data = {
        username: $('#id_login_form-username').val(),
        password: $('#id_login_form-password').val(),
    ...
    $.ajax({
        ...
        success: function(json){
            if (json.success == true){
                ...
                $('#sign-in-form-message').html(json.message).show()
            }
        },
        error: function(){
            console.log('error')
...
function sign_up_form_submit(){
    ...
    data = {
        username: $('#id_register_form-username').val(),
        email: $('#id_register_form-email').val(),
        password1: $('#id_register_form-password1').val(),
        password2: $('#id_register_form-password2').val(),
        csrfmiddlewaretoken: $('input[name=csrfmiddlewaretoken]').val()
    }
    $.ajax({
        ...
        success: function(json){
            ...
            if (json.success == true){
                location.href = 'main/thanks'
            } else {
                $('#sign-up-form-message').html(json.message).show()
            }
        },
        error: function(){
            console.log('error')
...
function forgot_password_form_submit(){
     ...
     data = {
        email: $('#id_forgotpass_form-email').val(),
        csrfmiddlewaretoken: $('input[name=csrfmiddlewaretoken]').val()
    }
    $.ajax({
        url: "/en/accounts/reset",
        type: 'POST',
        content_type: 'application/json',
        data: data,
        success: function(json){
            if (json.success == true){
                ...
                $('#forgot-password-form-message').html(json.message).show()
            }
        },
        error: function(){
            console.log('error')
...
function click_check(){
	...
	$('#htlfndr-forgot-password-form').on('click', 'a', function(event){
         $('#htlfndr-forgot-password').trigger('click');
	});
	$('#htlfndr-sing-up-form').on('submit',function(event){
		 event.preventDefault();
		 sign_up_form_submit();
	});
	$('#htlfndr-sing-in-form').on('submit',function(event){
		 event.preventDefault();
		 sign_in_form_submit();
	});
	$('#htlfndr-forgot-password-form').on('submit', function(event){
         event.preventDefault();
...
function browser_width(){
	...
	*/
	...
	if ( htlfndr_window_width > 991 ) {
		...
		// Clear local storage for small desktops
		localStorage.clear();
	}
	...
	/* Progress steps. Count 'li' elements to set their width. */
	if ( $( '.htlfndr-steps' ).length ) {
		var count_steps = $( '.htlfndr-progress li' ).length;
		if ( count_steps > 0 ) {
			var step_width = 100 / count_steps;
			$( '.htlfndr-progress li' ).css( 'width', step_width + '%' );
		}

		// Remove only description text in the step block for small desktops
		var step_description = $( '.htlfndr-steps' ).find( '.htlfndr-step-description' );
		if ( htlfndr_window_width < 768 ) {
			step_description.css( 'display', 'none' );
		} else {
			step_description.css( 'display', 'inline-block' );
		}
	}
}
function calendar(){
...
function htlfndr_save_view( class_name ) {
	localStorage.setItem( 'view_class', class_name );
}

/* Function for getting data from local storage */
function htlfndr_apply_view_preferences( current_block, removed_class ) {
	if ( null !== localStorage.getItem( 'view_class' ) ) {
		var view_class = localStorage.getItem( 'view_class' );
		current_block.removeClass( removed_class );
		current_block.addClass( view_class );
		// If user previously selected a Grid view
		if ( 'col-md-4' == view_class ) {
			current_block.closest( '.htlfndr-search-result').removeClass( 'htlfndr-row-view' );
			current_block.closest( '.htlfndr-search-result').addClass( 'htlfndr-grid-view' );
		} else if ( 'col-md-12' == view_class ) { // If user previously selected a Row view
			$( '#htlfndr-grid' ).removeClass( 'htlfndr-active' );
			$( '#htlfndr-row' ).addClass( 'htlfndr-active' );
			current_block.closest( '.htlfndr-search-result').removeClass( 'htlfndr-grid-view' );
			current_block.closest( '.htlfndr-search-result').addClass( 'htlfndr-row-view' );
		}
	}
}
function slider(){
...
/* Tooltips for Change View buttons */
	...
	/* End initializing of OWL Carousel
	 * on the hotel single page (gallery + map)
	 * */
...
/* Function for verifying and adding sorting parameters,
...
/* Start initializing of OWL Carousel
 * on the hotel single page (gallery)
 * */
...
function test_django_login() {

    $('#login-form-button').click(function(e){
        e.preventDefault();

        data = {'username': $('#id_login_form-username').val(),
                'password': $('#id_login_form-password').val()}
